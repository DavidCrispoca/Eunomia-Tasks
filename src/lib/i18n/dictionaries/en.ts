import type { Dictionary } from "./es";

const en: Dictionary = {
  app: {
    name: "Eunomia Tasks",
    tagline: "Order, discipline and harmony for your day",
  },
  nav: {
    home: "Home",
    kanban: "Board",
    calendar: "Calendar",
  },
  common: {
    add: "Add",
    cancel: "Cancel",
    save: "Save",
    delete: "Delete",
    edit: "Edit",
    search: "Search tasks…",
    close: "Close",
    today: "Today",
    language: "Language",
    theme: "Theme",
    taskCount: (count: number) => (count === 1 ? "1 task" : `${count} tasks`),
    markDone: "Complete task",
  },
  kanban: {
    title: "Board",
    subtitle: "Organize your pending, in-progress and finished work.",
    newTask: "New task",
    emptyColumn: "Nothing here",
    emptyHint: "Drag tasks here or create a new one.",
    columns: {
      todo: "To do",
      doing: "In progress",
      done: "Done",
    },
    sortHint: "Sort by date",
    sortManual: "Manual",
    sortDueAsc: "By due date: soonest first",
    sortDueDesc: "By due date: latest first",
    sortCreatedDesc: "By added: newest first",
    sortCreatedAsc: "By added: oldest first",
  },
  task: {
    title: "Title",
    titlePlaceholder: "Write a title…",
    notes: "Notes",
    notesPlaceholder: "Add more context…",
    priority: "Priority",
    dueDate: "Due date",
    dueTime: "Due time",
    status: "Status",
    group: "Class",
    createTask: "Create task",
    editTask: "Edit task",
    newTask: "New task",
    deleteTask: "Delete task",
    deleteConfirm: "Delete this task?",
    removed: "Task deleted",
    startWork: "Let's work",
  },
  priority: {
    low: "Low",
    medium: "Medium",
    high: "High",
  },
  calendar: {
    title: "Calendar",
    subtitle: "Reserve focus blocks for your week.",
    unscheduled: "Unscheduled",
    unscheduledHint: "Drag a task onto the calendar to book a block.",
    schedule: "Schedule",
    noScheduled: "No blocks scheduled.",
    block: "Block",
    from: "From",
    to: "To",
    editBlock: "Edit block",
    deleteBlock: "Delete block",
    minutes: "min",
    google: {
      connect: "Connect Google Calendar",
      connectHint:
        "Publish your blocks to your primary Google calendar and see Google events here.",
      connected: "Google Calendar connected",
      paused: "Sync paused",
      syncedAt: (time: string) => `Synced ${time}`,
      neverSynced: "Not synced yet",
      syncNow: "Retry",
      syncing: "Syncing…",
      pause: "Pause",
      resume: "Resume",
      disconnect: "Disconnect",
      disconnectConfirm:
        "Disconnect Google Calendar? Events already created in Google won't be deleted.",
      disconnectDone: "Google Calendar disconnected",
      syncError: "Could not sync with Google",
      syncPartial: (count: number) => `Synced with ${count} error(s)`,
      badge: "Google",
      openInGoogle: "Open in Google Calendar",
      timeAgo: {
        now: "just now",
        minutesAgo: (n: number) => `${n} min ago`,
        hoursAgo: (n: number) => `${n} h ago`,
        daysAgo: (n: number) => `${n} d ago`,
      },
    },
  },
  command: {
    placeholder: "Type a command or search…",
    goToHome: "Go to Home",
    goToKanban: "Go to board",
    goToCalendar: "Go to calendar",
    newTask: "Create new task",
    changeLanguage: "Change language to ",
    toggleTheme: "Toggle theme",
    toggleDone: "Toggle status of",
    noResults: "No results",
  },
  dashboard: {
    title: "Overview",
    subtitle: "Your metrics and upcoming deadlines at a glance.",
    focus: "Day focus",
    todayProgress: (done: number, total: number) =>
      `${done} of ${total} completed today`,
    nothingToday: "Nothing scheduled for today",
    pending: "Pending",
    overdueLabel: "Overdue",
    overdueAlert: (n: number) => `${n} tasks past deadline to catch up on`,
    thisWeek: "This week",
    completed: "Completed",
    completedToday: "completed today",
    dueSoon: "Due soon",
    dueSoonHint: "in the next 7 days",
    overdueHint: "tasks past deadline",
    weekLoad: "Week load",
    todayBlocks: "Today's blocks",
    noBlocksToday: "No blocks scheduled today",
    deadlines: "Upcoming deadlines",
    deadlinesEmpty: "No upcoming deadlines. Nice job!",
    deadlineMore: (count: number) => `+${count} more`,
    overdueBadge: "Overdue",
    todayBadge: "Today",
    tomorrowBadge: "Tomorrow",
    viewKanban: "Go to board",
    viewCalendar: "Go to calendar",
    noData: "Create your first task to start seeing your metrics.",
    focusByClass: "Focus by Class",
    focusByClassHint: "Pomodoro minutes accumulated per class.",
    focusEmpty:
      "No focus time recorded yet. Finish a pomodoro to get started.",
    focusMost: "Most focused",
  },
  empty: {
    noTasks: "You have no tasks yet",
    createFirst: "Create your first task with the button above.",
    allDone: "All done",
    allDoneHint: "Nothing left to do.",
  },
  pomodoro: {
    title: "Pomodoro",
    subtitle: "A focused block for a single task.",
    focus: "Focus",
    duration: "Block duration (min)",
    start: "Start",
    pause: "Pause",
    reset: "Reset",
    timeUp: "Time's up!",
    timeUpHint: "How did the session go?",
    keepDoing: "Keep in progress",
    markDone: "Mark as done",
    again: "Again",
    backToBoard: "Back to board",
    taskMissingHint: "This task doesn't exist or is no longer available.",
    finishNotifyBody: "Keep it in progress or mark it as done?",
    simple: "Simple",
    flight: "Flight",
    audio: "Ambient sound",
    audioNone: "No sound",
    audioWhite: "White noise",
    audioRain: "Rain",
    audioEngine: "Engine",
    volume: "Volume",
    byMinutes: "By minutes",
    byRoute: "By route",
    desiredMinutes: "Desired minutes",
    routeSuggestions: "Routes closest to your time",
    noRoutes: "No close routes. Try another time.",
    origin: "Origin",
    destination: "Destination",
    selectAirport: "Choose an airport…",
    flightDuration: "Flight duration",
    startFlight: "Take off",
  },
  account: {
    openMenu: "Open account menu",
  },
  group: {
    title: "Manage classes",
    all: "All",
    personal: "Personal",
    manage: "Classes",
    namePlaceholder: "e.g. Math",
    add: "Add",
    empty: "Create your first class to start organizing your tasks.",
    deleteGroup: "Delete class",
    deleteConfirm: (name: string) =>
      `Delete class "${name}"? Its tasks will move to Personal.`,
  },
  auth: {
    loginTitle: "Welcome back",
    loginSubtitle: "Sign in to continue managing your day.",
    registerTitle: "Create your account",
    registerSubtitle: "Order, discipline and harmony for your day.",
    name: "Name",
    namePlaceholder: "Your name…",
    email: "Email",
    emailPlaceholder: "you@email.com",
    password: "Password",
    passwordPlaceholder: "At least 6 characters",
    submitLogin: "Sign in",
    submitRegister: "Create account",
    toRegister: "No account?",
    registerWord: "Sign up",
    toLogin: "Already have an account?",
    loginWord: "Sign in",
    demoHint:
      "Local demo mode: use any email and a password of at least 6 characters.",
    demoBadge: "Demo",
    logout: "Sign out",
    errors: {
      invalid: "Invalid email or password.",
      exists: "An account with that email already exists.",
      generic: "Something went wrong. Please try again.",
      notAllowed: "This email is not authorized. The app is for personal use.",
    },
  },
  emailAdd: {
    title: "Add task by email",
    subtitle:
      "This link is temporary and only lets you add a task to your board, without signing in.",
    badge: "Signed link",
    successTitle: "Task added!",
    successBody:
      "The task is now on your Eunomia Tasks board. Close this page or sign in to view and organize it.",
    addAnother: "Add another task",
    loginNow: "Sign in",
    invalidTitle: "Invalid link",
    invalidBody:
      "This link is invalid, expired or already used. Request a new one from the email you received from Eunomia Tasks.",
    backToApp: "Back to home",
    noteRequired: "Write a title for the task.",
    noteSaved: "It will be saved to «To do» on your board.",
    submitError: "Could not create the task. Check the fields and try again.",
  },
};

export default en;