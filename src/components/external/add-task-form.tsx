    ...
    return (
      <div className="surface-gold-gradient relative overflow-hidden rounded-xl border border-white/10 shadow-[var(--inset-top),var(--app-shadow-lg)]">
        ...
        <div className="relative p-7 text-center">
          ...
          <Link
            href="/"
            className="mt-5 inline-flex h-8 items-center rounded-lg border border-white/10 bg-surface-2 px-3 text-sm text-foreground transition-colors hover:border-amber-500/30 hover:bg-surface-hover"
          >
      ...
      <div className="surface-gold-gradient relative overflow-hidden rounded-xl border border-white/10 shadow-[var(--inset-top),var(--app-shadow-lg)]">
        ...
        <div className="relative p-7 text-center">
          ...
          <div className="mt-6 flex flex-col gap-2.5">
            ...
            <Link
              href="/login"
              className="inline-flex h-11 w-full items-center justify-center rounded-lg border border-white/10 bg-surface-2 px-3.5 text-sm text-foreground transition-colors hover:border-amber-500/30 hover:bg-surface-hover"
            >
              {t.emailAdd.loginNow}
            </Link>
          </div>
  ...
  return (
    <div className="surface-gold-gradient relative overflow-hidden rounded-xl border border-white/10 shadow-[var(--inset-top),var(--app-shadow-lg)]">
      ...
      <div className="relative p-7">
        <div className="mb-6">
          <span className="mb-2 inline-flex items-center rounded-full border border-amber-500/25 bg-amber-500/10 px-2.5 py-0.5 text-[11px] font-medium text-amber-200/90">
            {t.emailAdd.badge}
          </span>
          <h1 className="text-[20px] font-semibold tracking-tight">
        ...
        <form onSubmit={submit} className="flex flex-col gap-3.5">
          ...
          {status === "error" && (
            <p className="rounded-lg border border-red-500/20 bg-red-500/10 px-3 py-2 text-[12px] leading-relaxed text-red-200/90">
              {t.emailAdd.submitError}
            </p>
          )}

          <Button type="submit" variant="primary" size="lg" disabled={status === "submitting" || !title.trim()}>
            {status === "submitting" ? "…" : t.common.save}
          </Button>

          <p className="text-center text-[11px] text-muted">{t.emailAdd.noteSaved}</p>
        </form>
      </div>
    </div>
  );
